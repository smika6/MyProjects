...
def main():
    global selectable, solved, delay
    #SETUP PYGAME
    pygame.init()
    pygame.font.init()
...
def min_k_means(game_window):
    global POINTS, CUTOFF, CLUSTER_COLORS, k, unlabeled, CLUSTER_CENTERS
    ...
    print(f"Time to compute min K Means: {t2 - t1:0.4f} seconds" )
...
def k_means(k, game_window):
    ...
    print(f"Time to compute K Means: {t2 - t1:0.4f} seconds" )
...
def controls(event):
    global user_input, CUTOFF, k_accepted, k, POINT_COUNT, POINTS
        ...
        if mouse[0] < WIDTH + HUD_WIDTH - HALF_GAP and mouse[0] > WIDTH + HUD_WIDTH - HALF_GAP - button_width:
            ...
            #button 3: Genetic
                ...
                if selectable[5]:
                    selectable[5] = False
                    POINTS.clear()
                    input_data_set("dataset.csv",POINTS, white_color)
